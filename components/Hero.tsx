...
import { Search, MapPin, Home, Building2, Users, Trees, ChevronDown } from 'lucide-react'

const cities = ['All Cities', 'Mumbai', 'Delhi NCR', 'Bengaluru', 'Pune', 'Hyderabad', 'Chennai', 'Kolkata', 'Ahmedabad']
...
const tabs = [
  {
    ...
    filterLabel: 'Budget',
    filterOptions: ['Any Budget', 'Under ₹50L', '₹50L - ₹1Cr', '₹1Cr - ₹2Cr', 'Above ₹2Cr'],
  },
  {
    id: 'rent',
    label: 'Rent',
    ...
    filterOptions: ['Any Rent', 'Under ₹15k', '₹15k - ₹30k', '₹30k - ₹50k', 'Above ₹50k'],
  },
  {
    id: 'pg',
    label: 'PG / Co-living',
    ...
    filterOptions: ['Any', 'Single', 'Double Sharing', 'Triple Sharing'],
  },
  {
    id: 'commercial',
    label: 'Commercial',
    ...
    filterOptions: ['Any Type', 'Office', 'Shop', 'Warehouse', 'Showroom'],
  },
  {
    id: 'plots',
    label: 'Plots / Land',
    ...
    filterOptions: ['Any Size', 'Under 1000 sq.ft', '1000-3000 sq.ft', 'Above 3000 sq.ft'],
  },
]
...
export default function Hero() {
  ...
  const [filter, setFilter] = useState(tabs[0].filterOptions[0])
  ...
  const handleTabChange = (tabId: string) => {
    ...
    setFilter(tab.filterOptions[0])
  }

 return (
    <section className="relative">
      {/* Background wrapper — overflow-hidden contained here only, not on the whole section */}
      <div className="absolute inset-0 overflow-hidden">
        ...
        />
        {/* Softer overlay so the photo stays visible, with a darker band at the bottom for text contrast */}
      ...
      <div className="relative max-w-5xl mx-auto px-4 pt-14 pb-16 md:pt-20 md:pb-24 text-center">
        <h1 className="font-display text-2xl sm:text-3xl md:text-5xl font-bold text-white leading-tight drop-shadow-sm">
          ...
          <br />
          <span className="text-brand-amber">Anywhere in India</span>
        </h1>
        ...
        <div className="mt-8 md:mt-10 bg-white rounded-2xl shadow-2xl p-2 md:p-3 text-left">
          ...
          {/* Search fields — change based on active tab */}
          <div className="flex flex-col md:flex-row gap-2 p-2">
            ...
            <div className="relative md:w-44">
              <button
                ...
                className="w-full flex items-center gap-2 border border-gray-200 rounded-xl px-3 py-2.5"
              >
              ...
              {cityOpen && (
                <div className="absolute top-full mt-1 left-0 w-full bg-white border border-gray-200 rounded-xl shadow-lg z-20 max-h-56 overflow-y-auto">
                  {cities.map((c) => (
                    <button
                      key={c}
                      onClick={() => {
                      ...
                      className="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-brand-sky"
                    >
                      {c}
                    </button>
                  ))}
            ...
            {/* Search input — placeholder changes per tab */}
            <div className="flex items-center gap-2 border border-gray-200 rounded-xl px-3 py-2.5 flex-1">
              ...
              <input
                ...
                className="text-sm text-gray-700 placeholder:text-gray-400 outline-none w-full"
              />
            </div>

            {/* Dynamic filter dropdown — changes per tab */}
            <div className="relative md:w-40">
              ...
              {filterOpen && (
                <div className="absolute top-full mt-1 left-0 w-full bg-white border border-gray-200 rounded-xl shadow-lg z-20 max-h-56 overflow-y-auto">
                  {currentTab.filterOptions.map((opt) => (
                    <button
                      key={opt}
                      onClick={() => {
                        setFilter(opt)
            ...
            <button className="bg-brand-amber text-brand-navy font-semibold text-sm px-6 py-2.5 rounded-xl hover:brightness-105 transition">
              Search
            </button>
          </div>
        </div>
